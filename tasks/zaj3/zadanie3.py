...
def generate_ngrams(contents, ngram_len=7, nmax=None):
	"""
	...
	:param int ngram_len: Długość generowanych n-gramów. Jeśli parametr ten
		...
		pojawiania się poszczególnych liter w wikipedii
...
def save_ngrams(out_file, contents):
	...
	with open(out_file, 'w') as f:
		...
		w.writerows(sorted(generate_ngrams(contents).items(), key = lambda x: x[0]))
