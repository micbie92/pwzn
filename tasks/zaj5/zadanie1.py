...
def load_data(path):
	...
	dtype = np.dtype([ ('ngram', np.dtype('a7')), ('counts', np.uint32) ] )
...
def suggester(input, data):
	...
	a = np.searchsorted(data['ngram'], [input, next])
	found = data[a[0]:a[1]]

	s = sum([ x[1] for x in found ])
	
	return sorted([ (chr(x[-1]), y/s) for x,y in found ], key = lambda x: x[1], reverse=True)
